...
const logoUrls = {
  // Türk takımları
  'galatasaray': 'https://img.sporekrani.com/tr_w_36,h_36,c_limit/teams/GG15p6Gkt4vy5CbGDmFNuSLHMC5ojiP0GatfJCdh.png',
...
const parseMatchTeams = (matchName) => {
  ...
  const separators = [' vs ', ' - ', ' x ', ' v ', ' VS ', ' X ', ' V ', ' : ', ' | ']
  let teams = [matchName]
  
  for (const sep of separators) {
    if (matchName.includes(sep)) {
      teams = matchName.split(sep).map(team => team.trim()).filter(team => team.length > 0)
      break
    }
  }
          ...
          <div className="lg:col-span-2 space-y-3 lg:space-y-4">
            <div className="bg-slate-800 rounded-lg overflow-hidden border border-green-500/30">
              {selectedMatch ? (
                  ...
                  <div className="bg-gradient-to-r from-green-600 to-emerald-600 px-3 py-2 lg:px-4 lg:py-3">
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        <h2 className="font-bold text-white text-sm lg:text-base truncate">{selectedMatch.name}</h2>
                        <span className="text-xs lg:text-sm text-green-100">{selectedMatch.time}</span>
                      </div>
                      <div className="flex items-center gap-2 ml-2 flex-shrink-0">
                        <span className="text-xs text-green-100 bg-white/20 px-2 py-1 rounded-full">🔴 CANLI</span>
                ...
                <div className="relative aspect-video flex flex-col justify-center text-center overflow-hidden rounded-lg" style={{
                  backgroundImage: 'linear-gradient(135deg, rgba(15,23,42,0.95), rgba(30,41,59,0.9)), url("https://images.unsplash.com/photo-1574629810360-7efbbe195018?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80")',
              ...
              <div className="flex relative">
                <div 
                  className={`absolute top-1 bottom-1 bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/30 rounded-lg transition-all duration-300 ease-out ${
                    activeTab === 'matches' ? 'left-1 right-1/2 mr-0.5' : 'left-1/2 right-1 ml-0.5'
                  }`}
                ></div>
